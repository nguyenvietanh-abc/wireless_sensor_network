...
void Temperature_Measurement_Task(void * parameter){
    ...
    char sensor_data[32];
    sprintf(sensor_data, sizeof(sensor_data),"%.1f,%.1f", temperature, humidity);
            ESP_LOGI(GATTS_TABLE_TAG,"temperature: %.1f°C, Humidity: %.1f%%", 
            ...
            heart_rate_handle_table[IDX_CHAR_VAL_A], strlen(sensor_data),          // Độ dài thực tế của chuỗi
                (uint8_t *)sensor_data,       // Dữ liệu gửi đi
                false);
